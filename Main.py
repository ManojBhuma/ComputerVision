...
def uploadVideo():
    ...
    while True:
        ...
        (h, w) = frame.shape[:2]
        blob = cv2.dnn.blobFromImage(cv2.resize(frame, (240, 240)),0.007843, (240, 240), 127.5)
        net.setInput(blob)
        ...
        for i in np.arange(0, detections.shape[2]):
            ...
            if confidence > 0.2:
                ...
                if (confidence * 100) > 50:
                    ...
                    cv2.putText(frame, "Object detected ", (startX, y),cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLORS[idx], 2)
                    text.insert(END,"Object detected "+"\n")
...
title = Label(main, text='OBJECT TRACKING')
title.config(bg='black', fg='orange')
title.config(font=font)           
title.config(height=3, width=120)       
title.place(x=0,y=5)

font1 = ('MS Sans Serif', 14, 'bold')
...
webcamButton = Button(main, text=" Webcam Video Tracking", command=webcamVideo)
